...
$messages = DB::table('user_messages')->where('user_id', $userId)->get();
                    ...
                    <div class="col-12 mb-4">
                        <div class="card">
                                ...
                                <div>
                                    ...
                                    <!-- 
                                        <div class="row border p-2 align-items-center">
                                            ...
                                            <div class="col-1">
                                                <span href="#" class="btn btn-danger">Delete</span>
                                            </div>
                                            <div class="col-2">{{$comment->created_at}}</div>
                                        </div> -->
                                    <!-- @endforeach -->
                                </div>
                            ...
                            <div class="row justify-content-between align-items-center p-3">
                                <span class="col-3">
                                    <a href="#" class="text-decoration-none text-dark discussion-link btn btn-warning" data-discussion-id="{{ $discussion->id }}" data-description="{{ $discussion->description }}" data-content-id="{{ $discussion->content->id }}" data-comments="{{ $discussion->comments }}">View</a>
                                </span>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        discussionLinks.forEach(link => {
            link.addEventListener('click', function(event) {
                ...
                const titleElement = this.querySelector('.card-header .text-start');


                const discussionTitleText = titleElement ? titleElement.innerText : 'No title available';
                ...
                // discussionDescription.innerText = discussionDescriptionText;


                // console.log(discussionIdInput.value);
                var testData = {
                ...
                // displayComments(commentsContainer, discussionComments)
                // commentsContainer.innerHTML = '';
                // Populate the comments
                // discussionComments.forEach(comment => {
                //     commentsContainer.innerHTML += `<p>${comment.comment}</p>`;
                // });
    ...
    function displayComments(container, comments, authUserId, discussionId = null) {
        ...
        comments.forEach(comment => {
            ...
            </div>  <div class="col-2">${formattedDate}</div>` :
    ...
    // Assume messages is passed as a JSON-encoded string from the server
    ...
    if (messageContainer) {
        ...
        messages.forEach(function(message) {
            ...
            pTag.classList.add('alert', 'alert-danger'); // Add Bootstrap alert classes
